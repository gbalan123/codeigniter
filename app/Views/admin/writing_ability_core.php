	...
	<div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <em class="fa fa-tasks fa-fw"></em><?php echo lang('app.language_admin_tds_writing_ability'); ?>
            </div>
            <!-- /.panel-heading -->
           
            <div class="panel-body">
            <?php if(!empty($writing_core_ability_current)){ ?>
            <div>
                <label for="name" style="margin-top:10px">Select version for Writing ability:</label> 
                <select class="form-control" name="version_wr_core_ability" id="version_wr_core_ability">
                    <?php foreach($writing_core_ability_version_details as $wr_ab_version){ ?>
                        <option value="<?php echo $wr_ab_version->version; ?>" 
                            <?php echo ($wr_ab_version->version == $writing_core_ability_current['version']) ? "selected" : "";?> >
                ...
                <div id="content_load">
                    ...
                    <div class="table-responsive table-bordered fixed-panel">
                        <table class="table">
                            ...
                            <tbody id="core_wr_ab_values"></tbody>
                        </table>
                    </div>
                </div>
                <?php } else { ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        core_wr_ability("writing");
        
        $('#version_wr_core_ability').on('change', function () {
            core_wr_ability("writing");
            
        }); 
    });
    function core_wr_ability(type){
        version = $('#version_wr_core_ability :selected').val();
        var obj = {};
        obj.value = version;
        obj.type = type;
        obj.course = "core";
        ...
        $.post("<?php echo site_url('admin/sw_ability_version'); ?>", obj, function (data) {
            if (data) {
                $('.loading').hide();
                $('#content_load').show();
                $("#core_wr_ab_values > tr").remove();
                $('#text').text(data.notes);
                arr = JSON.stringify(data);
                var array = $.map(data.valid, function(value, index) {
                    $("#core_wr_ab_values").append("<tr><td>"+value.adjusted_score+"</td><td>"+value.ability_estimate	+"</td></tr>")
                });
            }
        }, "json");
    } 
