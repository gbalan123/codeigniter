                    ...
                    <div class="col-xs-12">
                       ...
                       if($post_type == "higher"){
                        ...
                        <div class="form-group">
                              ...
                              <textarea class="form-control" name="text_area_sp" id ="textarea_sp" rows="2" cols="20" required></textarea>
						</div>
                        <div class="pd_20">
	...
	<div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <i class="fa fa-tasks fa-fw"></i><?php echo lang('app.language_admin_tds_speaking_ability'); ?>
            </div>
            <!-- /.panel-heading -->
           
            <div class="panel-body">
            <?php if(!empty($speaking_ability_current)){ ?>
            <div>
                <label for="name" style="margin-top:10px">Select version for Speaking ability:</label>
                <select class="form-control" name="version_sp_cr_ability" id="version_sp_cr_ability">
                    <?php foreach($speaking_ability_core_details as $sp_ab_version){ ?>
                        <option value="<?php echo $sp_ab_version->version; ?>" 
                            <?php echo ($sp_ab_version->version == $speaking_ability_current['version']) ? "selected" : "";?> >
                    ...
                    <div id="content_load">
                        ...
                        <div class="table-responsive table-bordered fixed-panel">
                            <table class="table">
                                ...
                                <tbody id="sp_ab_core_values"></tbody>
                            </table>
                        </div>
                    </div>
                <?php } else { ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        core_sp_ability("speaking");        

        $('#version_sp_cr_ability').on('change', function () {
            core_sp_ability("speaking");            
        }); 
    });
    function core_sp_ability(type){
        version = $('#version_sp_cr_ability :selected').val();
        var obj = {};
        obj.value = version;
        obj.type = type;
        obj.course = "core";
        ...
        $.post("<?php echo site_url('admin/sw_ability_version'); ?>", obj, function (data) {
            if (data) {
                $('.loading').hide();
                $('#content_load').show();
                $("#sp_ab_core_values > tr").remove();
                $('#text').text(data.notes);
                arr = JSON.stringify(data);
                var array = $.map(data.valid, function(value, index) {
                    $("#sp_ab_core_values").append("<tr><td>"+value.adjusted_score+"</td><td>"+value.ability_estimate	+"</td></tr>")
                });
            }
        }, "json");
    } 
