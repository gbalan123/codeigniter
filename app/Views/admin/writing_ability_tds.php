...
<div class="row">
	<div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
            <?php if(!empty($writing_ability_current)){ ?>
            <div>
                <label for="name" style="margin-top:10px">Select version for Writing ability:</label> 
                <select class="form-control" name="version_wr_sc_ability" id="version_wr_sc_ability">
                    <?php foreach($writing_ability_version_details as $wr_ab_version){ ?>
                        <option value="<?php echo $wr_ab_version->version; ?>" 
                            <?php echo ($wr_ab_version->version == $writing_ability_current['version']) ? "selected" : "";?> >
                ...
                <div id="content_load">
                    ...
                    <div class="table-responsive table-bordered fixed-panel">
                        <table class="table">
                            ...
                            <tbody id="writing_ability_values"></tbody>
                        </table>
                    </div>
                </div>
                <?php } else { ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        stepcheck_wr_ability("writing");       

        $('#version_wr_sc_ability').on('change', function () {
            stepcheck_wr_ability("writing");
        });        
    });
    
    function stepcheck_wr_ability(type){
        version = $('#version_wr_sc_ability :selected').val();
        var obj = {};
        obj.value = version;
        obj.type = type;
        ...
        $.post("<?php echo site_url('admin/sw_ability_version'); ?>", obj, function (data) {
            if (data) {
                $('.loading').hide();
                $('#content_load').show();
                $("#writing_ability_values > tr").remove();
                $('#text').text(data.notes);
                arr = JSON.stringify(data);
                var array = $.map(data.valid, function(value, index) {
                    $("#writing_ability_values").append("<tr><td>"+value.adjusted_score+"</td><td>"+value.ability_estimate	+"</td></tr>")
                });
            }
        }, "json");
    } 
