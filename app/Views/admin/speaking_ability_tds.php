	...
	<div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div>
                ...
                <select class="form-control" name="version_sp_sc_ability" id="version_sp_sc_ability">
                    <?php foreach($speaking_ability_version_details as $sp_ab_version){ ?>
                ...
                <div id="content_load">
                    ...
                    <div class="table-responsive table-bordered fixed-panel">
                        <table class="table">
                            ...
                            <tbody id="speaking_ability_values"></tbody>
                        </table>
                    </div>
                </div>
                <?php } else { ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        stepcheck_sp_ability("speaking");        

        $('#version_sp_sc_ability').on('change', function () {
            stepcheck_sp_ability("speaking");            
        }); 

    });
    function stepcheck_sp_ability(type){
        version = $('#version_sp_sc_ability :selected').val();
        var obj = {};
        obj.value = version;
        obj.type = type;
        obj.lookup = "ability";
        ...
        $.post("<?php echo site_url('admin/sw_ability_version'); ?>", obj, function (data) {
            if (data) {
                $('.loading').hide();
                $('#content_load').show();
                $("#speaking_ability_values > tr").remove();
                $('#text').text(data.notes);
                arr = JSON.stringify(data);
                var array = $.map(data.valid, function(value, index) {
                    $("#speaking_ability_values").append("<tr><td>"+value.adjusted_score+"</td><td>"+value.ability_estimate	+"</td></tr>")
                });
            }
        }, "json");
    } 
