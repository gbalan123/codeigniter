                    ...
                    <div class="col-xs-12">
                        <?php echo form_open_multipart('admin/post_weighting', array('class' => 'form', 'role' => 'form', 'method' => 'post', 'id' => 'speaking_weighting_form')); ?>
                            <div class="form-group">
                                ...
                                <input type="hidden" name="type" value="tds"/>
                            </div>
                            <div class="form-group">
                            ...
                            <!-- <div class="form-group">
                                <label for="name"><?php //echo "Message for changes"; ?>:</label>
                                <textarea id="text_area_sp" name="text_area_sp" rows="3" cols="30" required>  </textarea>
                            </div> -->
                            <div class="pd_20">
                        ...
                        <?php echo form_close(); ?>
                                <a class="pull-right mt_5" href="<?php echo site_url('admin/download_csv/speakingweighting'); ?>" download>
                                    <?php echo lang('app.language_admin_tds_download_existing_csv'); ?>
                                </a>
...
<!-- Speaking Dropdown Section - Starts -->
<div class="row">
	<div class="col-md-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
            <?php if(!empty($speaking_weight_current)){ ?>
            <div>
                <label for="name" style="margin-top:10px">Select version for speaking weight:</label>
                <select class="form-control" name="version_sp_stepcheck" id="version_sp_stepcheck">
                    <?php foreach($speaking_version_details as $s_version){ ?>
                        <option value="<?php echo $s_version->version; ?>" 
                            <?php echo ($s_version->version == $speaking_weight_current['version']) ? "selected" : "";?> >
                ...
                <div id="content_load">
                    ...
                    <div class="table-responsive table-bordered fixed-panel">
                        <table class="table">
                            ...
                            <tbody id="speaking_weight_values"></tbody>
                        </table>
                    </div>
                </div>    
                <?php } else { ?>
...
<!-- Speaking Dropdown Section - Ends -->
...
<!-- Writing Dropdown Section - Starts -->
<div class="row">
	<div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
            ...
            <div class="panel-body">
            <?php if(!empty($writing_weight_current)){ ?>
            <div>
                <label for="name" style="margin-top:10px">Select version for writing weight:</label>
                <select class="form-control" name="version_wr_stepcheck" id="version_wr_stepcheck">
                    <?php foreach($writing_version_details as $wr_version){ ?>
                        <option value="<?php echo $wr_version->version; ?>" 
                            <?php echo ($wr_version->version == $writing_weight_current['version']) ? "selected" : "";?> >
                ...
                <div id="content_load1">
                    ...
                    <div class="table-responsive table-bordered fixed-panel">
                        <table class="table">
                            ...
                            <tbody id="writing_weight_values"></tbody>
                        </table>
                    </div>
                </div>
                <?php } else { ?>
...
<!-- Writing Dropdown Section - Ends -->

<?php include 'footer.php'; ?>
<script type="text/javascript">
    $(document).ready(function () {

        stepcheck_sp_weighting("speaking");
        stepcheck_wr_weighting("writing");

        $('#version_sp_stepcheck').on('change', function () {
            stepcheck_sp_weighting("speaking"); 
        });  

        $('#version_wr_stepcheck').on('change', function () {
            stepcheck_wr_weighting("writing"); 
        });
    });

    function stepcheck_sp_weighting(type){
        version = $('#version_sp_stepcheck :selected').val();
        var obj = {};
        obj.value = version;
        obj.type = type;
        obj.course = "stepcheck";
        ...
        $.post("<?php echo site_url('admin/sw_weighting_version'); ?>", obj, function (data) {
            if (data) {
                $('.loading').hide();
                $('#content_load').show();
                $("#speaking_weight_values > tr").remove();
                $('#text').text(data.notes);
                arr = JSON.stringify(data);
                var array = $.map(data.valid, function(value, index) {
                    $("#speaking_weight_values").append("<tr><td>"+value.qnumber+"</td><td>"+value.weight+"</td></tr>")
                });
            }
        }, "json");
    }
    function stepcheck_wr_weighting(type){
        version = $('#version_wr_stepcheck :selected').val();
        var obj = {};
        obj.value = version;
        obj.type = type;
        obj.course = "stepcheck";
        ...
        $.post("<?php echo site_url('admin/sw_weighting_version'); ?>", obj, function (data) {
            if (data) {
                $('.loading1').hide();
                $('#content_load1').show();
                $("#writing_weight_values > tr").remove();
                $('#text2').text(data.notes);
                arr = JSON.stringify(data);
                var array = $.map(data.valid, function(value, index) {
                    $("#writing_weight_values").append("<tr><td>"+value.qnumber+"</td><td>"+value.weight+"</td></tr>")
                });
            }
        }, "json");
    }
