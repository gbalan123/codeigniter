...
<?php
    ...
    $user_lastnameDatas     = "user_lastname";
    $user_firstnameDatas    = "user_firstname";
?>
<!-- NAVBAR
================================================== -->
...
<?php if( (count($url_element) == 1) || ( isset($url_element[3]) && in_array($url_element[3],$array_pages)) ) { ?>
  ...
  <?php }?>
   ...
   if ($this->acl_auth->logged_in()) {
           if (strlen(ucfirst($this->session->get($user_firstnameDatas)." ".$this->session->get($user_lastnameDatas))) > 5) {
               $striped_user = mb_substr(ucfirst($this->session->get($user_firstnameDatas)." ".$this->session->get($user_lastnameDatas)), 0, 5) . '...';
           } else {
               $striped_user = mb_substr(ucfirst($this->session->get($user_firstnameDatas)." ".$this->session->get($user_lastnameDatas)),0,5,'UTF-8');
           }
        ?>
       <div class="dropdown user_logout" id="userDropdown">
           <a class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
               <em class="fa fa-user fa-2x"></em> <span class="user_name"><?php echo $striped_user; ?></span>
           </a>
           <div class="dropdown-menu  dropdown-menu-right" aria-labelledby="dropdownMenuButton">
    ...
    <div class="modal-dialog modal-md modal_login">
        <div class="modal-content contact-form">
            <div class="modal-header">
                <button type="button" class="close btn_close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><em class="fa fa-times"></em></span></button>
