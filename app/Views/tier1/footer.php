...
<script type="text/javascript">
    ...
    $(function () {
		...
		function sitelogout() {
			window.location = "<?php echo site_url('tier1/logout'); ?>";
		}
		...
		$('input:radio[name=tier1_option]').click(function () {
			var tier1_option = $('input[type="radio"][name="tier1_option"]:checked').val();
			console.log('tier_id : '+tier1_option);
			if(tier1_option == 'administer'){
				$('#institute_list').show();
			}else{
				$('#institute_list').hide();
			}
		...
		$('#continue_btn_tier3').click(function () {
			...
			obj.tier_id = tier_id;
			$.ajax({
				url: "<?php echo site_url('tier1/redirect_school'); ?>",
				data: obj,
				dataType: 'json',
				type: 'POST',
		...
		$('input:radio[name=tier1_option]').click(function () {
			var selected_option = $('input[type="radio"][name="tier1_option"]:checked').val();
			...
			$.ajax({
				url: "<?php echo site_url('tier1/set_session'); ?>",
				data: obj,
				dataType: 'json',
				type: 'POST',
				success: function (result) {
		...
		var selected_tier_option = $('input[type="radio"][name="tier1_option"]:checked').val();
        ...
        $.ajax({
				url: '<?php echo site_url('tier1/get_zend_desk_url_changing'); ?>',
				type: 'POST',
