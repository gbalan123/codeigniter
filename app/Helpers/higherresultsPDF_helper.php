...
function generatehigherResultsZipPDF($values_higher_pdf = FALSE){
        ...
        <tr>
            ...
            <td align="right" width="50%">
                ...
                <?php if($values_higher_pdf['qr_code_url'] != "" && $values_higher_pdf['google_url'] != ""){
                    ...
                    ?>
                    <div style="margin:0px 23px 5px 0;" class="qr_pic"> <img style="width:100px" src="<?php echo @image_png($parts[0],$parts[1],'qrcodes_higher'); ?>"></div>
...
<?php
        ...
        $name = $values_higher_pdf['data']['id'];
        
        //Creating folder using taskid
        $dir = $bulk_download_path.$values_higher_pdf['task_id'];
        $dir1 = $bulk_download_path.$values_higher_pdf['task_id'].'/'.$values_higher_pdf['file_name'];
        $oldmask = umask(0);
        if (!is_dir($dir)){
            if (!mkdir($dir, 0777, TRUE)) {
                error_log(date('[Y-m-d H:i e]') . ' Higher folder fails '. $dir .PHP_EOL, 3, LOG_FILE_PDF);
            }else{
                error_log(date('[Y-m-d H:i e]') .'Higher folder created ' . $dir . PHP_EOL, 3, LOG_FILE_PDF);
                if (!mkdir($dir1, 0777, TRUE)) {
                    error_log(date('[Y-m-d H:i e]') . ' Higher folder fails '. $dir1 .PHP_EOL, 3, LOG_FILE_PDF);
                }else{
                    error_log(date('[Y-m-d H:i e]') .'Higher sub folder created ' . $dir1 . PHP_EOL, 3, LOG_FILE_PDF);
                }
            }
        }
        umask($oldmask);
        
        $path = $bulk_download_path.$values_higher_pdf['task_id'].'/'.$values_higher_pdf['file_name'].'/';
        //writing files inside folder using dompdf
        if ( !write_file($path . $name . '.pdf', $dompdf->output())){
            error_log(date('[Y-m-d H:i e]') .'Higher File not written' . $path . $name . '.pdf' . PHP_EOL, 3, LOG_FILE_PDF);
        }else{
            error_log(date('[Y-m-d H:i e]') .'Higher File written ' . $path . $name . '.pdf' . PHP_EOL, 3, LOG_FILE_PDF);
        }
    }
}