 ...
 * 
 */
class Tier2 extends BaseController {
        ...
        /* WP-1391 zendesk Tier1 user add if not */
            if(isset($this->zendesk_access) && $this->zendesk_access == 1){
                    ...
                    if($zendesk_wp_user == false && isset($is_active) && $is_active['status'] == 1){
                        @zendesk_user_create($this->zendesk_domain_url,$user_id,"Create","Tier2 Login");
                    }
                }
    ...
    function index() {
        if (!$this->acl_auth->logged_in()) {
            ...
            return redirect()->to(site_url('tier2/dashboard')); 
        }
    }
    /* dashboard function */
    public function dashboard() {
        ...
        echo view('tier2/header');
        echo view('tier2/menus', $data);
        echo view('tier2/dashboard', $data);
        echo view('tier2/footer'); 
    }

    /* logout from session function */
    function logout() {
    ...
    public function institute_list(){
        ...
        if($total_rows > 10){
            ...
            $this->pager->makeLinks($page+1, $perPage, $total_rows, 'default_full', $segment, 'pagination_tier2_institution_list');
            $offset = $page * $perPage;
            $pager = $this->pager;
        }
        $data = array(
	...
	/* tire user to school redirect function */
	public function redirect_school(){
		...
		if(null !== $this->request->getPost() && $this->request->getPost('tier_id') && $this->request->getPost('tier_id') != ''){
			...
			if ($query->getNumRows() > 0) {
				...
				$this->session->set('institute_name', $institute_name);$result = array('success' => '1');
	...
	/* Tire user profile update function */
	public function profile($password_change=FALSE) {
        ...
        if ($this->request->getPost('profile_submit')) {
            ...
            if (!$this->validate($rules)) {
                ...
                if ($this->usermodel->update_profile($profiledata)) {
                    ...
                    /* WP-1391 zendesk Tier2 profile update */
                    if(isset($this->zendesk_access) && $this->zendesk_access == 1){
                        $user_id = $userdata['user_id'];
                        zendesk_profile_update($user_id,"Tier2 Update");   
                    }
                    $this->session->setFlashdata('messages', lang('app.language_admin_profile_updated_success_msg'));
                    return redirect()->to(site_url('tier2/profile/'));
                }
            }
        }
         ...
         if ($this->request->getPost('changepass_submit')) {
             ...
             if (!$this->validate($rules)) {
                 ...
                 if (!$this->passwordhash->CheckPassword($this->request->getPost('current_password'), $user[0]->password)) {
                     ...
                     $this->data['validation'] = $this->validator;
                     return redirect()->to(site_url('tier2/profile/'.$this->session->get('tabpass')));
                 } else {
        ...
        echo view('tier2/header');
        echo view('tier2/menus', $this->data);
        echo view('tier2/profile', $this->data);
        echo view('tier2/footer');
    }
